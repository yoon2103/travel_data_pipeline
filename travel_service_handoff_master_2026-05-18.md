...
## 2026-05-24 Bounded Runtime Implementation Direction

- 현재부터는 설계/관찰 반복보다 작은 bounded runtime 구현/검증 중심으로 진행한다.
- proposal-only phase를 최소화하고 `tiny scoped runtime improvement -> smoke -> rollback-safe 검증` 순서로 진행한다.
- 단, global demote, global public-facility demote, global meal demote, hard exclusion, replacement, bounded replacement, first-place 변경은 계속 금지한다.
- 작은 구현은 반드시 family/support-slot 범위를 좁히고, smoke에서 `NO_COURSE`, `places_empty`, drift, side effect를 확인한 뒤 유지 여부를 판단한다.

## 2026-05-24 Gangnam Editorial Tiny Demote Phase1

- 강남 weak support candidate 중 public/training/culture-style repeated support에 대해서만 support-slot-only tiny demote를 적용했다.
- 적용 대상은 `국가무형유산전수교육관`, `전수교육관`, `국기원`, `태권도본부`, `슈피겐홀`, `한생연`, `실험누리과학관` 계열이다.
- score delta는 `0.028`이며 first-place, global public facility, global meal에는 적용하지 않는다.
- trace는 `gangnam_editorial_soft_demote_applied`, `gangnam_editorial_soft_demote_count`, `gangnam_editorial_soft_demote_delta`를 추가했다.
- backend-only deploy 후 강남 10회, 성수 5회, 북촌 5회 smoke를 수행했다.
- 전체 결과는 `NO_COURSE=0`, `places_empty=0`, 성수/북촌 side effect 없음이다.
- 다만 강남 weak public support 감소는 확인되지 않았다. tiny demote는 적용됐지만 후보풀이 충분하지 않으면 weak support가 계속 선택될 수 있다.
- rollback은 runtime stability 기준으로 필요하지 않지만, 다음 단계에서 global demote로 확대하면 안 된다.
- 상세 보고서는 `reports/gangnam_editorial_tiny_demote_phase1.md`에 기록했다.

## 2026-05-24 Gangnam Representative Support Reinforcement Phase1

- 강남 weak support 문제를 penalty 확대가 아니라 representative support candidate reinforcement 방식으로 개선했다.
- 적용 범위는 `selected_anchor_family_id == seoul_gangnam`인 support-slot only이며 first-place에는 적용하지 않는다.
- 보강 대상은 COEX/Starfield/별마당, 가로수길/신사동, 압구정/청담, 봉은사/선정릉, Platform-L, AB Cafe 계열이다.
- score delta는 `gangnam_representative_support_boost=+0.035`이며 기존 tiny editorial demote `-0.028`은 유지했다.
- 추가 trace는 `gangnam_representative_support_boost_applied`, `gangnam_representative_support_boost_count`, `gangnam_representative_support_boost`다.
- backend-only deploy 후 강남 12회, 성수 5회, 북촌 5회 smoke를 수행했다.
- 전체 결과는 `NO_COURSE=0`, `places_empty=0`, 성수/북촌 side effect 없음이다.
- 강남 weak public support는 직전 phase 대비 run당 `1.40 -> 1.17`로 일부 감소했지만 해결 수준은 아니다.
- `서울 선릉과 정릉`은 10/12회 support로 유지되어 representative heritage support는 살아 있으나, `국가무형유산전수교육관`과 generic meal support가 계속 남는다.
- 다음 단계는 penalty 확대가 아니라 support-slot assembly ordering 또는 same-family candidate depth 원인 확인이다.
- 상세 보고서는 `reports/gangnam_representative_support_reinforcement_phase1.md`에 기록했다.

## 2026-05-24 - Gangnam support-slot assembly ordering trace

- 현재 방향은 proposal 반복 최소화와 tiny scoped runtime implementation/smoke 유지다.
- 다만 강남 weak support는 penalty/boost 이후에도 제한적으로만 개선되어, 이번 단계에서는 추가 penalty나 replacement가 아니라 support-slot assembly observability를 강화했다.
- `course_builder.py`와 `recommendation_observability.py`에 trace-only assembly ordering을 추가했다.
- 추가 trace:
...
- 적용 범위는 강남 family support-slot trace 중심이며 score/filter/replacement/first-place behavior는 변경하지 않았다.
- backend-only deploy 완료:
...
- 운영 smoke:
  - `/api/regions` 200
  - `places` row count 26381 유지
  - migration 미실행
  - production places write 없음
  - saju container 영향 없음
- 강남 12회 runtime smoke:
  - HTTP 200: 12/12
  - NO_COURSE: 0
  - places_empty: 0
  - trace events: 48
- 관찰 결과:
  - `morning_2`에서 국가무형유산전수교육관이 12/12 selected.
  - 국가무형유산전수교육관은 weak demote 이후에도 candidate order 1위로 남음.
  - lunch/afternoon_cafe에서는 알라프리마, 진수사 등 generic meal support가 반복.
  - 서울 선릉과 정릉이 pool 1위로 들어오는 경우에는 representative heritage support로 정상 선택됨.
- 판단:
  - weak support 유지 원인은 replacement 부족보다 candidate ordering / slot fill timing / representative candidate depth 부족 가능성이 크다.
  - 다음 구현 전 `gangnam_representative_support_boost`가 generic meal 후보에 적용되는 범위를 재검토해야 한다.
- 아직 금지:
  - global demote
  - penalty expansion
...
## 2026-05-25 - Gangnam representative support boost taxonomy fix

- 강남 support-slot assembly trace 결과, `representative_support_not_selected`에 generic meal 후보가 포함되는 문제가 확인됐다.
- 이번 단계는 penalty 확대가 아니라 boost taxonomy 정밀화로 처리했다.
- `_gangnam_representative_support_boost`에서 meal semantic 후보는 기본적으로 boost 제외하도록 수정했다.
- 단, 후보명 자체가 명확한 representative texture를 포함하는 경우만 예외적으로 boost 유지한다.
- boost 유지 texture:
  - COEX / Starfield / 별마당
  - 신사동 / 가로수길
  - 압구정 / 로데오 / 청담
...
- 강남 12회 smoke 결과:
  ...
  - NO_COURSE: 0
  - places_empty: 0
  - boosted generic meal: 0
  - boost 유지 후보: 플랫폼엘, 서울 선릉과 정릉
- 성수 5회, 북촌 5회 side effect 없음.
- 남은 weak support는 boost taxonomy 문제가 아니라 support-slot candidate depth/order 문제로 판단한다.
- 계속 금지:
  - global demote
  - penalty expansion
  - replacement / bounded replacement
...
# 2026-07-08 - Nearby destination explorer phase1

- 신규 화면 `목적지 주변 둘러보기`를 기존 추천 화면과 분리해 추가했다.
- 신규 API는 read-only로 유지했다.
  - `GET /api/explore/destinations`
  - `GET /api/explore/nearby`
- 신규 UI 진입은 `#/explore`와 하단 탭 `둘러보기` 기준이다.
- 기존 추천 화면 직접 수정은 피했다.
  - `HomeScreen.jsx` 직접 수정 없음
  - `ConditionScreen.jsx` 직접 수정 없음
  - `CourseResultScreen.jsx` 직접 수정 없음
- `이 주변으로 여행 코스 만들기` CTA는 숙박 여부를 묻고 기존 조건 선택 흐름으로 넘긴다.
  - `아니오`: 당일 여행 조건 흐름
  - `예`: 숙박지 기준 2일 이상 코스 metadata를 붙여 조건 흐름 진입
- 운영시간 데이터는 현재 restored DB에서 비어 있으므로 `운영 정보 확인 필요`를 기본 표시한다.
- 실제 코스 생성은 신규 explorer API와 분리된 기존 추천 엔진 영역이다. 임의 목적지 좌표에서 `NO_COURSE_ALL_RADIUS`가 반복되면 별도 bounded fallback QA로 다룬다.
- 검증:
  - backend compile PASS
  - `/api/explore/destinations` 200
  - `/api/explore/nearby` 200
