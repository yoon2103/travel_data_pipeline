...
def review_candidate(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if not reviewer:
        raise ReviewCandidateError("reviewer is required")
    ...
    try:
        ...
        if not candidate:
            raise ReviewCandidateError(f"candidate_id does not exist: {args.candidate_id}")
        ...
        new_payload = build_review_payload(
        ...
        with conn.cursor() as cur:
            cur.execute(
                """
                UPDATE place_enrichment_candidates
                SET review_status = %s,
                    review_payload = %s,
                    updated_at = NOW()
                WHERE candidate_id = %s
                  AND review_status IN ('PENDING_REVIEW', 'IN_REVIEW')
                RETURNING candidate_id, review_status, promote_status
                """,
                (
                    target_status,
        ...
        if not updated:
            conn.rollback()
            raise ReviewCandidateError(f"candidate_id could not be reviewed: {args.candidate_id}")
        conn.commit()
        return {
            "status": "UPDATED",
            "dry_run": False,
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Approve, reject, or skip a staged place enrichment candidate.")
